...
namespace RestaurantBillCalculator
{
    public partial class BillForm : Form
    {
        ...
        // === Menu Dictionaries (from Deitel Fig. 15.52) ===
        private readonly Dictionary<string, decimal> beverages = new()
        {
            { "(none)", 0.00m }, { "Soda", 1.95m }, { "Tea", 1.50m },
            { "Coffee", 1.25m }, { "Mineral Water", 2.95m },
            { "Juice", 2.50m }, { "Milk", 1.50m }
        };

        private readonly Dictionary<string, decimal> appetizers = new()
        {
            { "(none)", 0.00m }, { "Buffalo Wings", 5.95m }, { "Buffalo Fingers", 6.95m },
            { "Potato Skins", 8.95m }, { "Nachos", 8.95m }, { "Mushroom Caps", 10.95m },
            { "Shrimp Cocktail", 12.95m }, { "Chips and Salsa", 6.95m }
        };

        private readonly Dictionary<string, decimal> mains = new()
        {
            { "(none)", 0.00m }, { "Seafood Alfredo", 15.95m }, { "Chicken Alfredo", 13.95m },
            { "Chicken Picatta", 13.95m }, { "Turkey Club", 11.95m }, { "Lobster Pie", 19.95m },
            { "Prime Rib", 20.95m }, { "Shrimp Scampi", 18.95m }, { "Turkey Dinner", 13.95m },
            { "Stuffed Chicken", 14.95m }
        };

        private readonly Dictionary<string, decimal> desserts = new()
        {
            { "(none)", 0.00m }, { "Apple Pie", 5.95m }, { "Sundae", 3.95m },
            { "Carrot Cake", 5.95m }, { "Mud Pie", 4.95m }, { "Apple Crisp", 5.95m }
        };
        ...
        public BillForm()
        {
            ...
            // Load menu options
            BindCombo(cboBeverage, beverages);
            BindCombo(cboAppetizer, appetizers);
            BindCombo(cboMainCourse, mains);
            BindCombo(cboDessert, desserts);
        ...
        {
            decimal subtotal = 0m;
            subtotal += GetSelectedPrice(cboBeverage, beverages);
            subtotal += GetSelectedPrice(cboAppetizer, appetizers);
            subtotal += GetSelectedPrice(cboMainCourse, mains);
            subtotal += GetSelectedPrice(cboDessert, desserts);
